...
class _DateTimePickerWidgetState extends State<DateTimePickerWidget> {
  ...
  late FixedExtentScrollController _dayScrollCtrl, _hourScrollCtrl, _minuteScrollCtrl, _secondScrollCtrl;
  ...
  Widget _renderPickerView(BuildContext context) {
    ...
    if (widget.pickerTheme.showTitle) {
  ...
  void _onSelectedChange() {
    if (widget.onChange != null) {
      DateTime day = _baselineDate.add(Duration(days: _currDay));
      DateTime dateTime = DateTime(day.year, day.month, day.day, _currHour, _currMinute, _currSecond);
      widget.onChange?.call(dateTime, _calcSelectIndexList());
    }
  }
  ...
  Widget _renderDatePickerWidget() {
    ...
    Widget dayPickerColumn = _renderDatePickerColumnComponent(
      ...
      flex: dayFlex,
      itemBuilder: (BuildContext context, int index) => _renderDayPickerItemComponent(_dayRange.first + index, dayFormat),
    );
  ...
  Widget _renderDatePickerColumnComponent({
    ...
    Widget columnWidget = Container(
      ...
      child: CupertinoPicker.builder(
        ...
        childCount:
            format.contains('m') ? _calculateMinuteChildCount(valueRange, minuteDivider ?? 0) : valueRange.last - valueRange.first + 1,
        itemBuilder: (context, index) {
          int value = valueRange.first + index;

          if (format.contains('m')) {
            value = minuteDivider ?? 0 * index;
          }
